...
PIP_INDEX_URL=https://pypi-proxy.yourcompany.com/simple
# Optional Maven proxy passed to the Spark container at run time (blank = Maven Central)
MAVEN_PROXY=https://maven-proxy.yourcompany.com/maven2
```
...
> note: This honors `PIP_INDEX_URL` from your `.env`. You can also build the image directly with
> `docker build -t marimo-spark .`, optionally passing `--build-arg PIP_INDEX_URL=https://pypi-proxy.your-company.com/simple`.
...
| `local` | Enables the `local-uc` profile: a bundled `unitycatalog` server **plus** `marimo-spark`, wired together for an all-in-one local stack. |
...
> note: `just jars` resolves through `MAVEN_PROXY` (from your `.env`) when it is
...
> `repo1.maven.org` / `spark-packages` defaults first. With `MAVEN_PROXY` blank,
